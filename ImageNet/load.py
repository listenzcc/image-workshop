...
class ImageNetDataSet(object):
    ...
    def search_folder(self):
        ...
        for e in tqdm(self.root.iterdir(), 'Searching...'):
            if e.is_dir():
                [images.append((e.name, f.name)) for f in e.iterdir() if f.is_file() and f.suffix=='.JPEG'] 
            
            if e.suffix == '.xml':
                xmls.append((e.name,))

        images = pd.DataFrame(images, columns=['wnid', 'fileName'])
        images['cid'] = images['wnid'].map(lambda wnid: self.dataset.wnid_to_idx[wnid])
    ...
    def getitem(self, i: int=0):
        ...
        mass = self.wnid2mass(select['wnid'])

        # --------------------
        img = Image.open(Path(self.root, select['wnid'], select['fileName.jpeg']))
        ...
        # --------------------
        mass.update(img=img, tree=tree, xmls=xmls, uid=select['uid'])
        return mass
    ...
    def class2mass(self, class_name:str) -> dict:
    ...
    def wnid2mass(self, wnid:str) -> dict:
...
if __name__ == '__main__':
    ...
    inds = ImageNetDataSet()

    got = inds.getitem(random.choice(range(50000000)))
    display(got)
    ...
    for i, xml in enumerate(got['xmls']):
        wnid= xml['name']
        text = '{} | {}'.format(inds.wnid2mass(wnid)['classes'][0], inds.wnid2mass(wnid)['cid'])
        draw.rectangle((xml['xmin'], xml['ymin'], xml['xmax'], xml['ymax']), outline=color, width=2)
        left, top, right, bottom = draw.textbbox((xml['xmin'], xml['ymin']-15), text=text, font=font, align='left')
        draw.rectangle((left, top-2, right+8, bottom), fill=color)
        draw.text((xml['xmin']+4, xml['ymin']-15), text=text, font=font, align='left')
    display(img)
